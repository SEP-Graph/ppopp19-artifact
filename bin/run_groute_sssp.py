...
SSSP_BINARY_PATH = join(GROUTE_BUILD_PATH, "sssp")
...
for data_dirname, metadata in parse_all_metadata().items():
    ...
    prio_delta = metadata["Groute_sssp_prio_delta_fused"]

    timestamp = str(int(time()))
    log_path = join(LOG_ROOT, "Groute_sssp_%s_%s.log" % (data_dirname, timestamp))
    cmd = "%s" \
          " -graphfile=%s" \
          " -num_gpus=1" \
          " -startwith=1" \
    ...
    print('Evaluating SSSP implemented on Groute for "%s" dataset' % data_dirname)
