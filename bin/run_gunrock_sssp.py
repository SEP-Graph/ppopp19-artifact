...
SSSP_BINARY_PATH = join(GUNROCK_BIN_PATH, "sssp")
...
for data_dirname, metadata in parse_all_metadata().items():
    ...
    log_path = join(LOG_ROOT, "Gunrock_sssp_%s_%s.json" % (data_dirname, timestamp))
    cmd = "%s" \
          " market %s" \
          " --device=0" \
          " --undirected" \
    ...
    print('Evaluating SSSP implemented on Gunrock for "%s" dataset' % data_dirname)
    # run the program
